...
Entity::Entity()
    : _id(NULL_ENTITY), _archetypeId(NULL_ARCHETYPE), _indexInArchetype(0), _active(false) {}
...
Entity::Entity(EntityId id, ArchetypeId archetypeId, uint32_t indexInArchetype)
    : _id(id), _archetypeId(archetypeId), _indexInArchetype(indexInArchetype), _active(true) {}
...
uint32_t Entity::getIndexInArchetype() const
{
    return _indexInArchetype;
}
...
void Entity::setIndexInArchetype(uint32_t index)
{
    _indexInArchetype = index;
}
...
ArchetypeSignature::ArchetypeSignature(const std::vector<std::type_index> &types)
...
{
    auto it = std::lower_bound(_componentTypes.begin(), _componentTypes.end(), type);
...
{
    return std::find(_componentTypes.begin(), _componentTypes.end(), type) != _componentTypes.end();
}
...
size_t ArchetypeSignature::size() const
{
    return _componentTypes.size();
}

bool ArchetypeSignature::empty() const
{
    return _componentTypes.empty();
}
