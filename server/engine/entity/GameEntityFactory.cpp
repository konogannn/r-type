                                       ...
                                       float x, float y)
{
    ...
    _entityManager.addComponent(player, NetworkEntity(player.getId(), EntityType::PLAYER));
...
Entity GameEntityFactory::createEnemy(Enemy::Type type, float x, float y)
{
    ...
    _entityManager.addComponent(enemy, NetworkEntity(_nextEnemyId++, static_cast<uint8_t>(type)));
                                             ...
                                             const Position& ownerPos)
{
    ...
    _entityManager.addComponent(bullet, NetworkEntity(_nextBulletId++, EntityType::PLAYER_MISSILE));
