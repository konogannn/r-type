...
template <typename T>
void ComponentManager::addComponent(ArchetypeId archetypeId, uint32_t index, T &&component)
{
    ...
    archetype->addComponent(std::type_index(typeid(T)), std::move(componentPtr), index);
}
...
T *ComponentManager::getComponent(ArchetypeId archetypeId, uint32_t index)
{
    ...
    Component *comp = archetype->getComponent(std::type_index(typeid(T)), index);
...
template <typename T>
ArchetypeId ComponentManager::getArchetypeWithAddedComponent(ArchetypeId currentArchetypeId)
{
...
template <typename T>
ArchetypeId ComponentManager::getArchetypeWithRemovedComponent(ArchetypeId currentArchetypeId)
{
    Archetype *currentArchetype = getArchetype(currentArchetypeId);
    if (!currentArchetype)
    {
